...
def homepage():
    ...
    # Image QEJasmine
    blob = bucket.blob("others/QEJasmine.jpeg")
    QEJasmine = blob.generate_signed_url(datetime.timedelta(seconds=300), method="GET")
    response = requests.get(QEJasmine)
    image_data = response.content
    image = Image.open(BytesIO(image_data))
    QEJasmine = image.resize((500, 500))
