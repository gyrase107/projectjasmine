...
def homepage():
    ...
    # Video qe_jasmine
    blob = bucket.blob("others/qe_jasmine_vid.mov")
    qe_jasmine_vid = blob.generate_signed_url(datetime.timedelta(seconds=300), method="GET")
    response = requests.get(qe_jasmine_vid)
    video_data = response.content
    qe_jasmine_vid = resize(video, width=500, height=500)

    st.markdown("<h7 style='text-align: left; font-size: 15px;'></h7>", unsafe_allow_html=True)
    st.markdown("<h7 style='text-align: left; font-size: 15px;'>Below are some photos of myself, and some were generated by AI. Do you like them? ( ͡° ͜ʖ ͡° )</h7>", unsafe_allow_html=True)
    st.image(qe_jasmine)
    st.video(qe_jasmine_vid)
